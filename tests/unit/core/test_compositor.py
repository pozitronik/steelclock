...
@pytest.fixture
def compositor(mock_layout_manager, mock_api):
...
def test_compositor_init_default_values(mock_layout_manager, mock_api):
...
def test_compositor_init_custom_values(mock_layout_manager, mock_api):
...
def test_compositor_start(compositor):
...
def test_compositor_start_already_running(compositor):
...
def test_compositor_stop(compositor):
...
def test_compositor_stop_not_running(compositor):
...
def test_compositor_is_running(compositor):
...
def test_compositor_stop_with_timeout(compositor):
...
def test_compositor_render_frame_success(mock_layout_manager, mock_api):
...
def test_compositor_render_frame_logs_every_100_frames(mock_layout_manager, mock_api):
...
def test_compositor_render_frame_gamesense_api_error(mock_layout_manager, mock_api):
...
def test_compositor_render_frame_generic_error(mock_layout_manager, mock_api):
...
def test_compositor_render_frame_rate_limit_error_logging(mock_layout_manager, mock_api):
...
def test_compositor_render_loop_executes_frames(mock_layout_manager, mock_api):
...
def test_compositor_render_loop_stops_on_event(compositor):
...
def test_compositor_render_loop_handles_errors(mock_layout_manager, mock_api):
    """Тест что render loop продолжает работу при ошибках."""
    comp = Compositor(mock_layout_manager, mock_api, refresh_rate_ms=10)

    call_count = [0]

    def composite_side_effect():
        call_count[0] += 1
        if call_count[0] == 2:
...
def test_compositor_render_loop_sleeps_on_many_errors(mock_layout_manager, mock_api):
...
def test_compositor_render_loop_timing(mock_layout_manager, mock_api):
...
def test_compositor_get_stats(compositor):
...
def test_compositor_get_stats_after_rendering(mock_layout_manager, mock_api):
...
def test_compositor_get_stats_with_errors(mock_layout_manager, mock_api):
...
def test_compositor_context_manager(mock_layout_manager, mock_api):
...
def test_compositor_context_manager_with_exception(mock_layout_manager, mock_api):
...
def test_compositor_context_manager_auto_cleanup(mock_layout_manager, mock_api):
...
def test_compositor_zero_refresh_rate(mock_layout_manager, mock_api):
...
def test_compositor_very_high_refresh_rate(mock_layout_manager, mock_api):
...
def test_compositor_multiple_start_stop_cycles(compositor):
...
def test_compositor_thread_daemon_mode(compositor):
...
def test_compositor_empty_event_name(mock_layout_manager, mock_api):
